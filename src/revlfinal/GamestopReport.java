...
public class GamestopReport {
    public String reportGenerationStatus = "";
    public ObservableList<VideoGame> reportList;
    public GamestopReport(ObservableList<VideoGame> list)
    ...
    public String execute() throws Exception {
        ...
        userLocation.setPromptText("Gamestop Report");
    ...
    public void generateFile(TextField username, File fileLocation, ObservableList<VideoGame> reportList) {
        ...
        style.setFont(font);
        HSSFSheet sheet = workbook.createSheet("Gamestop Report");
        ...
        for (VideoGame item : reportList) {
            if (item.getSearching().equals("Yes")) {
                priceChange = Double.valueOf(item.getGamestop()) - Double.valueOf(item.getPreviousGamestop());
                priceChange = Precision.round(priceChange, 2, BigDecimal.ROUND_HALF_UP);
                count++;
                data.put(count, new Object[]{item.getName(), item.getGamestop(), item.getPreviousGamestop(), item.getPreviousGamestopDate(), priceChange});
            }
        }
        System.out.println("");
        ...
        try {
            ...
            reportGenerationStatus = "Gamestop Report Generated";
